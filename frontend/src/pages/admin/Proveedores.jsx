...
const Proveedores = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.nombre.trim() || !formData.documento.trim() || !formData.contacto.trim() || !formData.telefono.trim()) {
      setErrorMsg('El documento, nombre comercial, contacto y teléfono son obligatorios');
      return;
    }
  ...
  const handleToggleStatus = async (s) => {
    ...
    try {
      const res = await API.put(`/suppliers/${s._id}`, { ...s, estado: newStatus });
      if (res.data.ok) {
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 overflow-hidden shadow-sm">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 flex-wrap gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 font-light mt-0.5">Historial de proveedores de compras</p>
          </div>
          ...
          <table className="w-full text-left whitespace-nowrap">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Documento / NIT</th>
              ...
              {loading ? (
                ...
                currentItems.map(s => (
                  <tr key={s._id} className="hover:bg-slate-50/50 transition">
                    <td className="px-6 py-4 text-xs font-mono text-slate-600">{s.documento}</td>
                    <td className="px-6 py-4 font-semibold text-slate-800">{s.nombre}</td>
                    <td className="px-6 py-4 text-slate-600">{s.contacto || '—'}</td>
        ...
        {filteredSuppliers.length > 0 && (
          <div className="flex items-center justify-between px-6 py-4 bg-slate-50/50 border-t border-slate-100 flex-wrap gap-2 text-xs font-medium text-slate-500">
            <span>Mostrando {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredSuppliers.length)} de {filteredSuppliers.length} resultados</span>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-scale-up border border-slate-100">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">NIT / Documento *</label>
                <input 
                  type="text" 
                  name="documento"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Contacto / Representante *</label>
                <input 
                  type="text" 
                  name="contacto"
